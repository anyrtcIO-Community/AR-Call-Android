...
public class ARVideoView implements View.OnTouchListener{
    ...
    protected  class VideoView {
        ...
        public VideoView(final String videoId, final Context ctx, EglBase eglBase, int index, final int x, int y, int w, int h) {
...
//        }
        ...
        LocalVideoRender.surfaceViewRenderer.addFrameListener(new EglRenderer.FrameListener() {
            ...
            public void onFrame(Bitmap frame) {
                ...
                LocalVideoRender.surfaceViewRenderer.post(new Runnable() {
                    @Override
                    public void run() {
                        LocalVideoRender.flLoading.setVisibility(GONE);
                    }
                });
    ...
    public VideoRenderer openProVideoRender(String videoId) {
        VideoView remoteVideoRender = allVideoList.get(videoId);
        if (remoteVideoRender == null) {
            remoteVideoRender = new VideoView(videoId, rlVideoGroup.getContext(), eglBase, 1, 35, 18, 30, 20);
            ...
            remoteVideoRender.surfaceViewRenderer.addFrameListener(new EglRenderer.FrameListener() {
                ...
                public void onFrame(Bitmap frame) {
                    finalRemoteVideoRender.surfaceViewRenderer.post(new Runnable() {
                        @Override
                        public void run() {
                            finalRemoteVideoRender.flLoading.setVisibility(GONE);
                        }
                    });
                }
            }, 1f);
